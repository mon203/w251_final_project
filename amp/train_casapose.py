...
def smooth_l1_loss(
    ...
    in_loss = optimizer.get_loss_scale(tf.where(smoothL1_sign, tf.square(vertex_diff) * 0.5, (vertex_diff - 0.5)))
...
def proxy_voting_dist(
    ...
    in_loss = optimizer.get_loss_scale(
        tf.reduce_sum(
...
def proxy_voting_loss_v2(
    ...
    if loss_per_object and normalize:
        ...
        in_loss = optimizer.get_loss_scaling(
            tf.reduce_sum(
                tf.where(tf.less(dist, 1.0), tf.square(dist) * 0.5, (dist - 0.5)), axis=-1
            )
        )  # [b, h, w, vn]
        ...
        in_loss = optimizer.get_loss_scaling(tf.where(tf.less(dist, 1.0), tf.square(dist) * 0.5, (dist - 0.5)))  # [b, h, w, vn]
        if normalize:
            in_loss = tf.reduce_sum(tf.reshape(in_loss, [b, -1]), 1) / (
                ver_dim * tf.reduce_sum(tf.reshape(vertex_weights, [b, -1]), 1) + 1e-3
...
def compute_loss(
    ...
    mask_loss = optimizer.get_scaled_loss(tf.constant(0.0, dtype=tf.float32))
    vertex_loss = optimizer.get_scaled_loss(tf.constant(0.0, dtype=tf.float32))
    proxy_loss = optimizer.get_scaled_loss(tf.constant(0.0, dtype=tf.float32))
    ...
    if loss_factors.mask_loss_weight > 0.0:
        loss = tf.nn.softmax_cross_entropy_with_logits(labels=target_seg, logits=output_seg)
        scaled_loss = optimizer.get_scaled_loss(loss)
        mask_loss = tf.reduce_mean(scaled_loss)
    ...
    if loss_factors.vertex_loss_weight > 0.0:
        if separated_vectors:
            vertex_loss = sum(smooth_l1_loss(
    ...
    if loss_factors.proxy_loss_weight > 0.0:
        if separated_vectors:
            proxy_loss = sum(proxy_voting_loss_v2(
                        output_vert[:, :, :, i * vc : (i + 1) * vc],
                        target_points[:, i : i + 1, :, :, :],
    ...
    if kp_loss is None:
        kp_loss = optimizer.get_scaled_loss(tf.constant(0.0, dtype=tf.float32))

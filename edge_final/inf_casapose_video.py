...
# img = cv2.imread("/casapose/casapose_input/001235.png")
...
while True:
    ...
    test_dataset = ImageOnlyDataset(frame)

    test_dataset = VectorfieldDataset(
        root="/casapose/data/datasets/lm/test2", # Hardcoded param
    ...
    testingdata, test_batches = test_dataset.generate_dataset(
        1, 1, 0, (480, 640), 1.0, 1, no_objects, shuffle=False
    )

    mesh_vertex_array, mesh_vertex_count = test_dataset.generate_object_vertex_array()

    testingdata_iterator = iter(testingdata)
    frame = runnetwork(testingdata_iterator, int(test_batches))
    print(type(frame))

    now = datetime.now()
    dt_string = now.strftime("%Y-%m-%d-%H-%M-%S")

    save_frames = f"casapose_output/frame{dt_string}.png"
    cv2.imwrite(save_frames, frame)
